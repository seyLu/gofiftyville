...
func Interviews(f InterviewsFilter) ([]Interview, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("-> (1) model.Interviews %+v : %w", f, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&interview.ID, &interview.Name, &interview.Year, &interview.Month, &interview.Day, &interview.Transcript); err != nil {
			return nil, fmt.Errorf("-> (2) model.Interviews %+v : %w", f, err)
		}
		interviews = append(interviews, interview)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("-> (3) model.Interviews %+v : %w", f, err)
	}
